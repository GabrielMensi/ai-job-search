...
describe("htmlFetch", () => {
  test("passes an AbortSignal timeout to fetch", async () => {
    ...
    await htmlFetch("https://www.simplyhired.com.ar/search");
    expect(init?.signal).toBeInstanceOf(AbortSignal);
  });

  test("returns null on 404 instead of throwing", async () => {
    globalThis.fetch = (async () => new Response("", { status: 404 })) as unknown as typeof fetch;
    expect(await htmlFetch("https://www.simplyhired.com.ar/job/nope")).toBeNull();
  });
  ...
  test("retries on 429 then succeeds", async () => {
    ...
    const html = await htmlFetch("https://www.simplyhired.com.ar/search");
