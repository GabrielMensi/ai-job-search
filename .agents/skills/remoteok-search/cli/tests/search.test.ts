...
// Live smoke test against the real RemoteOK API (low volume: a couple of requests).
describe("live search smoke test", () => {
  test("search for a common tag returns real results", async () => {
    ...
    expect(first.url).toMatch(/^https:\/\/remote(ok\.com|OK\.com)\//i);
