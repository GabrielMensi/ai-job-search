...
**`linkedin/` documents:** About/summary section (full text, used for behavioral inference), work experience, education, skills and endorsements, **Languages section** (language name + self-rated proficiency level, e.g. "Spanish - Native or bilingual proficiency" - a high-confidence structured source, feeds the Deal-Breaker Gate in `04-job-evaluation.md`), certifications, volunteer work, publications, recommendations received (full text). If multiple LinkedIn exports are present, use the most recently modified file.
...
- Languages you work in professionally, with proficiency levels (only if not already extracted from `cv/` or `linkedin/` above) - this feeds the Deal-Breaker Gate in `04-job-evaluation.md`, so ask directly rather than skipping it
...
- **Job portals:** Use Glob (`.agents/skills/*/SKILL.md`) to list the portal skills actually installed in this fork, and read each one's `enabled:` frontmatter line so what you present matches reality (don't assume upstream's shipped set — a fork may have added, removed, or pre-disabled portals). Present them in two groups, since they serve different purposes:
  - **Market portals** (already `enabled: true` by default) - the country-agnostic CLIs (`linkedin-search`, `freehire-search`) plus whatever local-market boards this fork ships (e.g. Argentina/LatAm boards, or the Danish demos on a vanilla clone). Confirm these match the user's market; if their market isn't covered, guide them to `/add-portal`.
  - **English-heavy portals, opt-in only** - any portal shipped `enabled: false` by default because most of its postings expect a strong/fluent English level regardless of the user's local market (e.g. this fork's `latojobs-search`/`wearedistributed-search`/`remoteok-search`, aimed at US/global companies hiring LatAm talent). **Do not enable these silently or assume the user wants them** - ask directly: "This fork also ships [names], boards for [US/global] companies hiring remotely, but most of their postings expect strong or fluent English. Your declared English level is [level from the Languages table, once captured] - want me to turn these on?" If yes, edit each chosen skill's `SKILL.md` frontmatter (`enabled: false` -> `enabled: true`) directly. If the user's English level isn't yet captured at this point in the interview, ask the language question first (see the Languages field elsewhere in this section) so the recommendation is grounded rather than a guess - and if they decline or are unsure, leave them off; enabling is reversible any time by re-running `/setup --section search` or editing the flag by hand.
  - Are there other job boards the user uses that aren't shipped at all? Guide them to `/add-portal`. WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
