...
│   ├── simplyhired-ar-search/         # SimplyHired Argentina (Indeed network - disabled by default, robots.txt)
│   ├── latojobs-search/               # LatoJobs (US companies hiring in LatAm - disabled by default)
│   ├── wearedistributed-search/       # We Are Distributed (US companies hiring in LatAm - disabled by default)
│   ├── remoteok-search/               # RemoteOK.com (global remote, English-heavy - disabled by default)
...
This fork additionally ships ten Argentina/LatAm portal CLIs, scaffolded the same way via `/add-portal`, in two groups:
...
**`simplyhired-ar-search`** (`enabled: false` by default, for its own separate reason — see below): [simplyhired.com.ar](https://www.simplyhired.com.ar), Argentina (Indeed/Recruit Holdings network). `__NEXT_DATA__` JSON, no HTML scraping. `ar.indeed.com` itself is behind a Cloudflare interactive challenge that also fingerprints the TLS handshake (confirmed live: even a replayed session cookie from the same machine still got `403`), so it isn't shipped directly — this skill is the practical substitute, since SimplyHired shares Indeed's underlying job inventory (confirmed live via `indeedApply`/`dateOnIndeed` fields, real company diversity, and a majority-organic, not just sponsored, result set) without any of that protection. Off by default specifically because its `robots.txt` names `anthropic-ai` in a blanket-disallow block separate from the generic rules this CLI's browser-UA requests actually fall under (a deliberate, documented judgment call — see `url-reference.md` for the full text) — enabling it is a call this repo shouldn't make on your behalf. Also note a bare search isn't supported (redirects to a Cloudflare-challenged homepage).

**"US companies hiring in LatAm" portals** (`enabled: false` by default — most postings on these expect a strong/fluent English level regardless of the user's local market, so `/setup`'s Job Portals question asks before turning any of these on rather than assuming everyone wants them; enable by hand any time by flipping `enabled: false` → `true` in the skill's `SKILL.md` frontmatter, or re-run `/setup --section search`):
